...
```{r}
ggplot(by_cat, aes(x = dev.category, y = `sum(weights_rounded)`)) +
...
- do a side by side bar graph that compares proportion of variables from each category in ours/original formula
...
weights_by_cat <- dict %>%
  group_by(category) %>%
  summarize(sum(weights_rounded)) %>%
  print()

print(us_formula_weights)
print(predictions)
```