...
selected_vars <- names(which(best_model_step))[-c(1, 2, 3, 4, 5, 6)]
selected_vars <- c(selected_vars, "LOCALE", "CIP15BACHL", "CIP39BACHL", "CIP50BACHL")
which(selected_vars == "CIP15BACHL1") 
which(selected_vars == "CIP39BACHL1")
which(selected_vars == "CIP50BACHL1")

# refit model with selected variables
formula <- as.formula(paste("Ranking ~", paste(selected_vars, collapse = " + ")))
...
predictions_step <- predict(final_model_step, newdata = testing[, selected_vars])
